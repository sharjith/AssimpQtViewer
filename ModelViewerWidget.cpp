	...
	m_isDragging(false), m_mode(InteractionMode::Select), m_multiSelectionEnabled(false)
{
    ...
    format.setSamples(4); // Set MSAA samples
    setFormat(format);
    ...
    connect(m_viewToolbar, &ViewToolbar::viewSelected, this, [this](const QString& view) {
        if (view == "Top") setViewTop();
        else if (view == "Front") setViewFront();
        else if (view == "Left") setViewLeft();
        else if (view == "Bottom") setViewBottom();
        else if (view == "Rear") setViewRear();
        else if (view == "Right") setViewRight();
    });

    connect(m_viewToolbar, &ViewToolbar::axonometricSelected, this, [this](const QString& type) {
        if (type == "Isometric") setViewAxonometric();
        else if (type == "Dimetric") setViewDimetric();
        else if (type == "Trimetric") setViewTrimetric();
    });

    connect(m_viewToolbar, &ViewToolbar::fitToViewRequested, this, &ModelViewerWidget::fitToView);

    connect(m_viewToolbar, &ViewToolbar::projectionToggled, this, [this](bool ortho) {
        m_camera->setProjectionType(ortho ? GLCamera::ProjectionType::ORTHOGRAPHIC : GLCamera::ProjectionType::PERSPECTIVE);
        update();
    });
...
void ModelViewerWidget::mouseMoveEvent(QMouseEvent *event)
{
    ...
    if (m_viewToolbar && event->buttons() == Qt::NoButton)
    {
        ...
        if (revealArea.contains(event->pos()) || m_viewToolbar->underMouse())
        {
