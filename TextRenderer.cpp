...
void TextRenderer::Load(std::string font, unsigned int fontSize)
{
	...
	if (!loadFontFromResource(QString(font.c_str()), ft, face))
	{
...
bool TextRenderer::loadFontFromResource(const QString& resourcePath, FT_Library& library, FT_Face& face)
{
	// Load the font from Qt's resource system
	QFile fontFile(resourcePath);
	if (!fontFile.open(QIODevice::ReadOnly))
	{
		qDebug() << "Failed to open font resource:" << resourcePath;
		return false;
	}
	...
	if (!tempFile.open())
	{
		qDebug() << "Failed to create temporary file for font.";
		return false;
	}

	// Write the font data to the temporary file
	tempFile.write(fontFile.readAll());
	tempFile.flush(); // Ensure all data is written to disk

	// Pass the temporary file's path to FreeType

	if (FT_New_Face(library, tempFile.fileName().toUtf8().data(), 0, &face))
	{
		qDebug() << "Failed to load font face:";
		return false;
	}
